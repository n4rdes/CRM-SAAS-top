...
export default async function WorkspaceLayout({ children }: { children: React.ReactNode }) {
  ...
  return <div className="workspace">
    ...
    <main className="workspace-main"><header className="workspace-topbar"><div className="topbar-context"><small>PRISMAE PEOPLE OS</small><strong>{tenant.name}</strong></div><div className="topbar-actions"><Link href="/app/agenda" className="topbar-agenda"><span>＋</span> Nova atividade</Link><Link href="/app/assinatura" className="topbar-plan">Plano {plan?.name ?? "Basic"}</Link><span className="workspace-user-avatar">{initial}</span></div></header>{attention && <div className="billing-alert">Sua assinatura precisa de atenção. <Link href="/app/assinatura">Ver cobrança →</Link></div>}{children}</main>
  </div>;
}
