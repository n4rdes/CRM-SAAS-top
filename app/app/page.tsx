...
function DashboardMetricIcon({ kind }: { kind: "clients" | "jobs" | "candidates" | "pipeline" | "people" | "goals" | "engagement" }) {
  const paths = {
...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<{ welcome?: string; success?: string }> }) {
  ...
  const [clients, jobs, candidates, applications, subscription, audit, activities, employees, performanceCycles, performanceGoals, performanceReviews, engagementSurveys, engagementActions] = await Promise.all([
  ...
  return <div className="workspace-content">
    ...
    <section className="metric-grid dashboard-metric-grid"><article className="metric-card"><DashboardMetricIcon kind="clients" /><small>Clientes no CRM</small><strong>{clients.count ?? 0}</strong><em>carteira comercial</em></article><article className="metric-card"><DashboardMetricIcon kind="jobs" /><small>Vagas ativas</small><strong>{jobs.count ?? 0}{typeof activeJobLimit === "number" ? `/${activeJobLimit}` : ""}</strong><em>limite do plano</em></article><article className="metric-card"><DashboardMetricIcon kind="candidates" /><small>Candidatos</small><strong>{candidates.count ?? 0}</strong><em>banco de talentos</em></article><article className="metric-card"><DashboardMetricIcon kind="pipeline" /><small>No pipeline</small><strong>{activeApplications}</strong><em>{hires} contratação(ões)</em></article><article className="metric-card"><DashboardMetricIcon kind="people" /><small>Headcount</small><strong>{employeeRows.length}{typeof planData?.limits?.employees === "number" ? `/${planData.limits.employees}` : ""}</strong><em>{preboarding} em admissão</em></article><article className="metric-card"><DashboardMetricIcon kind="goals" /><small>Metas</small><strong>{hasPerformance ? `${averageGoalProgress}%` : "PRO"}</strong><em>{hasPerformance ? `${pendingReviews} avaliação(ões) pendente(s)` : "desbloqueie desempenho"}</em></article><article className="metric-card"><DashboardMetricIcon kind="engagement" /><small>Clima & eNPS</small><strong>{hasEngagement && engagementSummary && !engagementSummary.locked && engagementSummary.enps !== null ? engagementSummary.enps : hasEngagement ? "—" : "PRO"}</strong><em>{!hasEngagement ? "desbloqueie escuta contínua" : currentEngagementSurvey ? `${engagementSummary?.participation ?? 0}% participação · ${openEngagementActions} ação(ões)` : "crie a primeira escuta"}</em></article></section>
    <section className="dashboard-panels"><article className="panel"><h2>Ações rápidas</h2><p>Cadastre dados ou abra os módulos completos.</p><div className="quick-links dashboard-quick-links"><Link href="/app/clientes"><span>01</span>Gerenciar clientes</Link><Link href="/app/vagas"><span>02</span>Pipeline de vagas</Link><Link href="/app/pessoas"><span>03</span>Gestão de Pessoas</Link><Link href="/app/desempenho"><span>04</span>{performanceCycles.data?.name ?? "Desempenho"}</Link><Link href="/app/clima"><span>05</span>Medir clima & eNPS</Link><Link href="/app/agenda"><span>06</span>Organizar agenda</Link></div></article><article className="panel subscription-summary"><h2>Assinatura</h2><p>{subscription.data?.status === "trialing" ? `Avaliação ativa${trialEndLabel ? ` · termina em ${trialEndLabel}` : ""}` : `Status: ${subscription.data?.status ?? "não encontrado"}`}</p><span className="plan-chip">{plan}</span></article></section>
