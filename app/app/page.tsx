...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<{ welcome?: string; success?: string }> }) {
  ...
  const [clients, jobs, candidates, applications, subscription, audit, activities, employees, performanceCycles, performanceGoals, performanceReviews] = await Promise.all([
  ...
  const planData = subscription.data?.plan as unknown as { name?: string; limits?: { active_jobs?: number | null; employees?: number | null } } | null;
