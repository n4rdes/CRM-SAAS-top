...
2. Execute, nesta ordem, todo o conteúdo de:
   ...
   - `supabase/migrations/202607230009_production_hardening.sql` — webhooks transacionais, rate limiting, auditoria de documentos e índices de produção.
...
Use Node.js 22 LTS (a versão está fixada em `.nvmrc` e `.node-version`). Se o PowerShell bloquear `npm.ps1`, use `npm.cmd`:
...
O ciclo de endurecimento está documentado em [`docs/PRODUCTION_HARDENING.md`](docs/PRODUCTION_HARDENING.md). Antes de publicar, aplique a migration `202607230009_production_hardening.sql`, configure rate limiting, valide RLS em staging e habilite um scanner privado para documentos sensíveis.
